...
const TablaReporte = React.memo(
  ({ datos, estructuras, hayBusqueda = false, search = "" }) => {
    ...
    if (!datos || !Array.isArray(datos) || datos.length === 0) {
      return (
        <EmptyState
          ...
          title={
            hayBusqueda ? "Sin coincidencias" : "Aún no hay reportes generados"
          }
          description={
            hayBusqueda
              ? `No se encontraron resultados que coincidan con "${search.trim()}". Intenta con otro término.`
            ...
            <tbody>
              {datos.map((item, idx) => (
                <tr key={`${item.Item}-${idx}`}>
                  ...
                  >
                    {Number(
                      item.Total_Exist_Und_Alternas || 0,
                    ).toLocaleString()}
                  </td>
                </tr>
              ))}
